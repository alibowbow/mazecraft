...
> 나만의 물길을 만드는 Water Atelier

MazeCraft는 3D 미로 물 시뮬레이션을 중심으로 한 로컬 퍼스트 웹앱입니다. 첫 화면에서 물을 흘리고, 미로·물색·유량·재질·조명을 조절할 수 있습니다. 기존 미로 편집기, 퍼즐 플레이, 파일 가져오기와 내보내기는 ‘내 미로’에서 이용합니다. 회원가입이나 서버 데이터베이스가 필요하지 않습니다.

## 주요 기능

- 전체 화면 물 스튜디오, 포슬린 가든·캐스케이드·트윈 플로우·리본·워터 가든 프리셋
- 연속 도자기 벽, 4단 수로, 유약 반사, 수면 아래 빛무늬, 따뜻한 질감 배경
- 5가지 미로 재질, 3가지 조명, 물색·유입량·농도·수면 움직임 튜닝과 설정 복원
- 2D/3D 보기, 자유 회전·확대, 재생·일시정지·배수, 모바일 튜닝 패널
- 물색과 재질 조절은 현재 물의 상태를 유지하며, 미로 구조 변경은 흐름을 다시 시작
...
활성 셀을 저장 control volume으로, 열린 통로를 동적 유량 edge로 모델링해 수두차·중력·마찰·출구 경계에 따라 실시간으로 흐름을 적분합니다. 그 위에 다중 스케일 방향성 수면과 잔류 포말을 합성합니다.

물리 단위, 경계조건, 안정화 방식과 질량 보존 진단은 [Water Dynamics 2.0 설계 문서](docs/water-dynamics-2.md)에 정리되어 있습니다.
