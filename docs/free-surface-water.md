...
- `renderer.ts` accumulates particle density on the GPU and reconstructs a continuous cyan boundary. All movement follows solver snapshots; appearance presets do not alter physics. Exact wall rectangles cover the fluid boundary, with subtle bevels and shadows. Pan, zoom and fit also work while paused.
