...
test('15.2 모바일 프로젝트 메뉴는 하나의 화면 내 액션 시트로 열린다', async ({ page }, testInfo) => {
  ...
  await expect(sheet.getByRole('button', { name: /미로 편집/ })).toBeVisible()
  await expect(sheet.getByRole('button', { name: /물 흘리기/ })).toBeVisible()
...
test('15.4 모바일 그리기는 벽 도구가 아니라 실루엣 획으로 저장된다', async ({ page }) => {
  ...
  await createBasic(page)
  await page.locator('.mobile-tabs').getByRole('button', { name: '모양', exact: true }).click()
