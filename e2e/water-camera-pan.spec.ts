...
test('3D 미로를 확대하고 드래그해도 멈춘 물리 상태는 변하지 않는다', async ({ page }, testInfo) => {
  ...
  // A right-button drag pans the view without orbiting it.
  const view = () => canvas.evaluate(element => ({ orientation: element.dataset.cameraOrientation, target: element.dataset.cameraTarget }))
  const orbited = await view()
  await page.mouse.down({ button: 'right' })
  await page.mouse.move(x - bounds!.width * 0.1, y - bounds!.height * 0.05, { steps: 10 })
  await page.mouse.up({ button: 'right' })
  const panned = await view()
  expect(panned.orientation).toBe(orbited.orientation)
  expect(panned.target).not.toBe(orbited.target)
  ...
  expect((await canvas.screenshot()).equals(settled)).toBe(false)
  expect(await readWaterState(stage)).toEqual(before)
})
