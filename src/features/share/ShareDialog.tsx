...
export function ShareDialog({ project, valid, onClose, mode = 'play' }: ShareDialogProps) {
  const [includeSolution, setIncludeSolution] = useState(false)
  const [includeReplay, setIncludeReplay] = useState(mode === 'play' && Boolean(project.creatorReplay))
  ...
  const result = useMemo(
    ...
    [allowRemix, includeReplay, includeSolution, project, mode],
  )
  ...
  const nativeShare = async () => {
    ...
    setError('')
    try {
      const status = await shareUrl({ title: project.title, text: mode === 'water' ? '물 미로의 흐름을 함께 살펴보세요.' : '이 미로에 도전해보세요.', url: result.url })
  ...
  return (
    <Dialog title={mode === 'water' ? '물 미로 공유' : '미로 공유'} onClose={onClose}>
      ...
      {result.ok ? (
        <>
          ...
          <button className="button secondary" onClick={() => window.open(result.url, '_blank', 'noopener,noreferrer')}><ExternalLink size={17} />{mode === 'water' ? '새 창에서 물 미로 보기' : '새 창에서 플레이 테스트'}</button>
        </>
      ) : (
        <div className="notice error">{result.message}</div>
      )}
      {mode === 'water' ? <p className="notice">미로 구조를 공유합니다. 물색과 재질은 각 기기의 설정을 사용합니다.</p> : <div className="settings-stack">
