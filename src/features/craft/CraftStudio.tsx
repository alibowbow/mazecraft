...
import {
  ...
  Flower2, Grid3x3, LayoutTemplate, Link, Map, Maximize2, Minus, Pause, Play, Plus, RotateCcw, Shuffle, Sparkles, Spline, Tornado, Trash2, Video, Waves, X, type LucideIcon,
} from 'lucide-react'
    ...
    <div className="ws-workspace">
      ...
      <aside className="ws-tuning craft-panel" aria-label="물길 크래프트">
        ...
        <div className="ws-panel-content craft-content">
          ...
          {!working && advice.length > 0 && <div className="craft-advice" role="group" aria-label="추천 해결 방법">
            <span className="craft-advice-label"><Sparkles size={13} />이렇게 고쳐 보세요 · 눌러서 바로 적용</span>
            {advice.map(fix => <button key={fix.id} onClick={() => apply(fix)}><strong>{fix.title}</strong><small>{fix.detail}</small></button>)}
          </div>}

          <button className="craft-presets-toggle" aria-expanded={presetsOpen} onClick={() => setPresetsOpen(!presetsOpen)}>
            <LayoutTemplate size={15} /><span>프리셋 둘러보기</span><small>{templates.length}개</small><ChevronDown size={14} />
          </button>
          {presetsOpen && <div className="craft-presets">
